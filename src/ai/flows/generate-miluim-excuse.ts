...
const generateMiluimExcuseFlow = ai.defineFlow(
  ...
  async input => {
    const { output } = await generateMiluimExcusePrompt(input);
    if (!output) {
        // Fallback in case the model doesn't call the tool
        const excuse = await excuseGeneratorTool();
        return { excuse };
    }
    const toolResponse = output.toolResponse;
    if (toolResponse && typeof toolResponse === 'string') {
        return { excuse: toolResponse };
    }
    
    // If the model replies but doesn't use the tool, generate one manually.
    const excuse = await excuseGeneratorTool();
    return { excuse };
  }
);
